...
d.  Write the estimated least squares regression line below using proper notation. $$\widehat{\text{weight}} = 7.27 - 0.59(\text{habitsmoker})$$ NOTE: I asked ChatGPT what the proper notation is to add in the "(habitsmoker)" bit, otherwise it looked weird.
...
$$\widehat{\text{parasite similarity}} = 0.379487 - 0.006532(\text{divergence_time})$$
...
Divergence time: 
$$\widehat{\text{transformed_parsim}} = 0.368 - 0.091(\text{divergence_time})$$

Distance:
$$\widehat{\text{transformed_parsim}} = -0.0529 - 0.00027(\text{distance})$$

BMdiff:
$$\widehat{\text{transformed_parsim}} = -2.696 - 0.000005(\text{BMdiff})$$

precdiff:
$$\widehat{\text{transformed_parsim}} = -2.532 - 0.006(\text{precdiff})$$
