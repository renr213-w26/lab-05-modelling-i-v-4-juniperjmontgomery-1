...
The regression line goes below zero for parasite similarity after ~58 million years, which is impossible since parasite similarity is a proportion between 0 and 1 that cannot be negative. This means that the line is model is only reliable for predicting up to ~58 million years. 
...
The graph shows a clear linear negative relationship between divergence time and log-transformed parasite similarity, where the regression line better fits through all the points compared to the untransformed data.
...
Divergence time: 
$$\widehat{\text{transparsim}} = 0.368 - 0.091(\text{divergetime})$$

Distance:
$$\widehat{\text{transparsim}} = -0.0529 - 0.00027(\text{distance})$$

BMdiff:
$$\widehat{\text{transparsim}} = -2.696 - 0.000005(\text{BMdiff})$$

precdiff:
$$\widehat{\text{transparsim}} = -2.532 - 0.006(\text{precdiff})$$
NOTE: the names for these are ugly (e.g. transparsim represents transformed_parsim), but I couldn't render the document unless I took out underscores and spaces in their names.
...
```
Divergence time R^2 = 0.5125147
Distance R^2 = 0.4432029
BM diff R^2 = 0.003269571
Prec diff R^2 = 0.008611603
...
```{r}
ggplot(gapminder_07, aes(x = gdpPercap_log, y = lifeExp, colour = continent)) +
  geom_point() +
  labs(
    x = "Log GDP per capita (USD, inflation-adjusted)",
    y = "Life Expectancy (years)",
